...
export default function UseReducerApp() {
    ...
    const Buttons = ( props ) => {
        return <button className="bg-green-500 w-[150px] p-2 text-center rounded-md duration-300 hover:bg-green-600 hover:cursor-pointer" {...props}>{props.children}</button>
    }
    console.info('Task adicionada:', state.tasksCount);

    return ( // <Buttons onClick={() => dispatch({ type: "decrement" })}>Decrement</Buttons>
        <div className="w-screen h-screen flex flex-col flex-wrap justify-center gap-6 items-center">
            ...
            <div className="flex gap-4">
                <input type="text" value={inputValue} onChange={(event) => setInputValue(event.target.value)} className="border-black rounded-md p-1 border-2 font-mono" />
                <Buttons 
                    onClick={() => {
            ...
            {state.tasks.map((task, index) => (<p className={`font-mono hover:cursor-pointer ${task.isCompleted ? 'line-through' : 'no-underline'}`} onClick={() => dispatch({ type: 'toogle-task', payload: index })}>{task.name}</p>))}
        </div>
    );
}